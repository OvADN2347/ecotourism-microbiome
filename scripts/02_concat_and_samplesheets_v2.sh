...
generate_block () {
  ...
  if (( ${#indices[@]} == 0 )); then
    echo "ERROR: No FASTQ files found in ${PROJECT_ROOT}/${INDIR}" >&2
    exit 1
  fi
  ...
  for n in "${indices[@]}"; do
    ...
    if [[ "$MARKER" == "Fung" ]]; then
      if (( ${#r1_files[@]} != 1 || ${#r2_files[@]} != 1 )); then
        echo "ERROR: Fung/ITS expected exactly one R1 and one R2 for index ${n}, but found R1=${#r1_files[@]} R2=${#r2_files[@]}" >&2
        exit 1
      fi
    ...
    local abs_r1 abs_r2 rel_r1 rel_r2
